...
public class RevokeInputFilter implements InputFilter {
    private final int MAX_ITEM_IN_STACK = 100;
    /**
     * 这里的栈中计划存储三种不同的操作符，分别是添加，删除和替换
     */
    private Stack<Action> mBackStack;
    ...
    public CharSequence filter(CharSequence source, int start, int end, Spanned dest, int dstart, int dend) {
        ...
        if (origin.startsWith(Action.IGNORE_SIGN) && origin.endsWith(Action.IGNORE_SIGN)) {
            return origin.substring(Action.IGNORE_SIGN.length(), origin.length() - Action.IGNORE_SIGN.length());
        }
        if (origin.startsWith(Action.REVOKED_SIGN) && origin.endsWith(Action.REVOKED_SIGN)) {
            //如果这时是一个回退操作
            return origin.substring(Action.REVOKED_SIGN.length(), origin.length() - Action.REVOKED_SIGN.length());
        } else {
    ...
    private void compressBackStack() {
        ...
        for (int i = 0; i < MAX_ITEM_IN_STACK / 10; i++) {
